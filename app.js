...
io.on("connection", function (socket) {
  ...
  socket.on("enterRoom", async ({ name, room }) => {
    ...
    if (prevRoom) {
      ...
      io.to(prevRoom).emit(
        "message",
        chatFeature.buildMsg(ADMIN, `${name} has left the room`)
      );
    }
    ...
    socket.emit(
      "message",
      chatFeature.buildMsg(ADMIN, `You have joined the ${user.room} chat room`)
    );
    ...
    socket.broadcast
      ...
      .emit(
        "message",
        chatFeature.buildMsg(ADMIN, `${user.name} has joined the room`)
      );
  ...
  socket.on("disconnect", () => {
    ...
    if (user) {
      io.to(user.room).emit(
        "message",
        chatFeature.buildMsg(ADMIN, `${user.name} has left the room`)
      );
